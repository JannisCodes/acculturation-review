...
lib <- c("rmarkdown", "knitr", "citr", "remedy", "bookdown", "papaja", "rmdfiltr", "psych",
         ...
         "DiagrammeR", "janitor", "DiagrammeRsvg", "rsvg", "manipulateWidget", "htmlwidgets", "webshot2", "boot")
...
### Situation  
Acculturation can happen in different life domains [e.g., @Arends-Toth2007; @Zane2004]. We coded which life domains the scales referred to within the scale items.  (see Figure \@ref(fig:ScaleDomainFreq)).  
...
We additionally inspected bi-variate relations between the individual life domains. We calculate the phi coefficient (for binary variables) together with the raw number of co-occurrences (see Table \@ref(tab:methodologicalDomainCooccurrences)).  
...
kEmp %>%
  kbl(., 
      ...
      align = c('l', rep('c', length(.)-1)),
      digits=3)  %>%
  footnote(general = paste0("$\\kappa_{pooled}$ = ",format(round(kEmpPooled$k.pooled, 2), nsmall=2), ", $95\\%CI_{boot}$[", format(round(kEmpPooled$lwr, 2), nsmall=2), ", ", format(round(kEmpPooled$upr, 2), nsmall=2), "]")) %>%
  kable_classic(full_width = F, 
                lightable_options = "hover", 
                html_font = "Cambria")
```
