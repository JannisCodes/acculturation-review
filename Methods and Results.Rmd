...
```{r ABCDFreq, fig.cap="Bar graph of the element combinations."}
...
DimensionFreq <- data.frame(Dimension = names(DimensionFreq), 
                            Frequency = DimensionFreq, 
                            Percentage = DimensionFreq/nrow(dt.Scales.Included)) %>%
  mutate(Dimension = fct_reorder(Dimension, Frequency))

# barplot of dimension frequency
ScaleABCD <- ggplot(data=DimensionFreq, aes(x=Dimension, y=Frequency)) +
  ...
  labs(title = "Frequency across all Scales",
...
sum(ABCD_comb %>% filter(Affect == 1) %>% select(Frequency))

# bar plot frequencies
ScaleABCDComb <- ggplot(ABCD_comb, aes(x=ExperienceCombination, y=Percentage)) +
  geom_bar(stat="identity", fill="grey14") +
  geom_text(
  ...
  scale_y_continuous(limits = c(0, ceiling(max(ABCD_comb$Percentage)*1.1)),
                     breaks = seq(0, ceiling(max(ABCD_comb$Percentage)*1.1), 5))+
...
ScaleABCDComb + 
  annotation_custom(
    ggplotGrob(ScaleABCD), 
    xmin = 0.5, xmax = 5, ymin = 17, ymax = 30
  )
```
