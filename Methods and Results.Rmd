...
lib <- c("rmarkdown", "knitr", "citr", "remedy", "bookdown", "papaja", "rmdfiltr", "psych", "xtable",
         ...
         "DiagrammeR", "janitor", "DiagrammeRsvg", "rsvg", "irr")
...
kappa.full <- function(x, name, ...){
  ...
  data.frame(name=name,Po=Po,Pe=Pe,k, se=se)
}
...
dt.Scales.Included %>%
  select(Affect, Affect2) %>%
  mutate_at(vars(Affect, Affect2), ~replace_na(., 0)) %>%
  irr::kappa2(., "unweighted")

dt.Scales.Included %>%
  select(Behavior, Behavior2) %>%
  mutate_at(vars(Behavior, Behavior2), ~replace_na(., 0)) %>%
  irr::kappa2(., "unweighted")

dt.Scales.Included %>%
  select(Cognition, Cognition2) %>%
  mutate_at(vars(Cognition, Cognition2), ~replace_na(., 0)) %>%
  irr::kappa2(., "unweighted")

dt.Scales.Included %>%
  select(Desire, Desire2) %>%
  mutate_at(vars(Desire, Desire2), ~replace_na(., 0)) %>%
  irr::kappa2(., "unweighted")
...
\paragraph{Context}
We additionally coded a range of contextual variables, including the type of sample collected (e.g., student, clinical), the life domain targeted (e.g., school, family), the cultures considered (i.e., host and migrant groups), the type of analysis conducted (e.g., correlation, regression), the measurement type (e.g., continuous scale, categorical classification), as well as the variable type of acculturation in the analysis (e.g., predictor, dependent). Further information as well as analyses of the contextual variables go beyond the scope of the immediate framework validation and are thus only presented in Supplemental Material C.
...
To further assess the comparative utility of the experience framework, we further assessed differences of experience aspects between academic fields. For the full results, including differences in the methods, and publication types as well as contextual differences in terms of sampling procedures, situational domains, analyses, and cultural contexts see Supplemental Material B. 
