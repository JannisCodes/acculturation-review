...
```{r FieldPlotComplexityAverage, include=F, fig.width=12, fig.height=16}
...
p <- ggplot(FieldComplexity, aes(y=Percentage, x=field_labs_lng, fill=fct_rev(as.factor(complexity))))+
  ...
  #stat_compare_means(data = PostHocField, comparisons = my_comparisons, label = "p.signif", label.y = c(1.05, 1.1), tip.length = 0.01) +
  scale_y_continuous(labels = scales::percent, breaks = seq(0,1,.2)) +
  geom_text(aes(label = paste0(round(Percentage,2),"%"), y = Percentage/100), position = position_stack(vjust = 0.5), size = 3) +
  labs(title = "Distributions Measurement Complexity",
       y = "Complexity",
       x = "Field",
...
ggstatsplot:::ggsignif_adder(
  ...
  y = Percentage,
  df_pairwise = df_pair,
  ggsignif.args = list(textsize = 3, tip_length = 0.0001)
)
