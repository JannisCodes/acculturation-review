...
rowPack <- c(rep(paste0("Theoretical (N = ",nrow(dt.Theories.Included), ")"),3), 
             rep(paste0("Methodological (N = ",nrow(dt.Scales.Included), ")"),3), 
             rep(paste0("Empirical (N = ",nrow(dt.psych.included), ")"),3)) %>%
  table(.) %>%
...
rbind(theoElementCor, scaleElementCor, empElementCor) %>%
  ...
  kable_styling(position = "left") %>%
  footnote(general = "Diagonal: Times aspect occurred; upper triangle: Phi Association Matrix; lower triangle: Times aspects co-occurred.") %>%
