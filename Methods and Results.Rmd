...
empiricalExclFull <- data.frame(table(Exclusion = dt.psych.abstract$NoteMissing)) %>%
  mutate(screening = "Full Text") %>%
  arrange(desc(Freq))
...
prismaGr(found = nrow(dt.psych),
         ...
         screen_exclusions = nrow(dt.psych.unique)-nrow(dt.psych.abstract), 
         full_text = nrow(dt.psych.abstract),
         full_text_exclusions = nrow(dt.psych.abstract)-nrow(dt.psych.fulltext), 
         qualitative = nrow(dt.psych.fulltext), 
         quantitative = nrow(dt.psych.included),
         extra_dupes_box = F,
         width = 800, height = 800) %>%
  export_svg() %>%
  charToRaw %>% 
...
At the most applied level, we assessed the broader empirical studies. This final database included the largest number of manuscripts and is in theory the application of the theoretical and methodological literature. The search produced a total of `r nrow(dt.psych)` results to which we added 133 articles through contacts with experts in the field and from referenced works within the review. After duplicate removal, title--, abstract--, and full text screening we coded a total of `r nrow(dt.psych.included)` empirical works (for exclusion reasons see Table \ref{tab:ExclusionsCombined} and for our PRISMA diagram see Figure \ref{fig:PRISMA}). 
