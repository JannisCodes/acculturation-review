...
# !IMPORTANT!
# BEFORE FIRST RENDER:
# To install all relevant packages please run "renv::restore()" in the console before the first use to ensure that all packages are using the correct version.
lib <- c("rmarkdown", "knitr", "citr", "remedy", "bookdown", "papaja", "rmdfiltr", "psych",
         ...
         "DiagrammeR", "janitor", "DiagrammeRsvg", "rsvg", "manipulateWidget")
...
## Data Import 
In a first step we import the raw data of the systematic review from the shared coding [Google Sheet](https://docs.google.com/spreadsheets/d/1j3j7q15lhNqPxp3qGnRtc2zuaE7plxWYR7tWKltkdU8/edit?usp=sharing). We, import the databases of theories and scale validations as well as the database of the empirical papers. Beyond that we also import parts of the codebook for list creations.  
...
manipulateWidget::combineWidgets(prismaTheory, prismaScales, prismaEmpirical, nrow=1)
```
...
dt.Theories.Included %>%
  dplyr::select(Theory, CitationKey,
                Affect, Behavior,	Cognition,	Desire,
                 FrameworkTheoryModel, GeneralAspect, Time,	SourceType) %>%
  mutate_at(vars(Affect, 	Behavior,	Cognition, Desire), ~replace_na(., 0)) %>%
  mutate(CitationKey = paste0("@",CitationKey)) %>%
...
dt.Scales.Included %>%
  mutate(SourceShort = stri_replace_all_fixed(Source,
                                         ...
                                         vectorize_all = FALSE)) %>%
  dplyr::select(Scale, CitationKey,
                Affect, 	Behavior,	Cognition,	Desire,
                `Source ^a^` = SourceShort, Sample,	IncludesMajority, 
                HostCountry,	OriginCountry) %>%
  mutate_at(vars(Affect, 	Behavior,	Cognition, Desire), ~replace_na(., 0)) %>%
  mutate(CitationKey = paste0("@",CitationKey)) %>%
...
#                                          replacement = c("CEL", "MAE", "MAT", "WAL", "ZAN", "OWN"), 
#                                          vectorize_all = FALSE)) %>%
#   dplyr::select(Scale, CitationKey, `Source [note]"` = SourceShort, 
#                 Affect, 	Behavior,	Cognition,	Desire,
#                 Sample,	IncludesMajority, 
#                 HostCountry,	OriginCountry) %>%
#   mutate_at(vars(Affect, 	Behavior,	Cognition, Desire), ~replace_na(., 0)) %>%
#   kbl(., 
#       label = "ScaleTbl",
...
dt.Empirical.included %>%
  dplyr::select(CitationKey, 
                Affect, 	Behavior,	Cognition,	Desire,
                PublicationType, Method, Sample,	IncludesMajority, MigrationTime,
                HostCountry,	OriginCountry) %>%
  mutate_at(vars(Affect, 	Behavior,	Cognition, Desire), ~replace_na(., 0)) %>%
  mutate(CitationKey = paste0("@",CitationKey)) %>%
  kbl(., caption = "Empirical Literature",
...
### Theory type
The authors of the `r nrow(dt.Theories.Included)` included theoretical works self-categorized their contributions as a theoretical conceptualization (\textit{N} = `r nrow(dt.Theories.Included %>% filter(FrameworkTheoryModel=="Conceptualization"))`), theoretical framework (\textit{N} = `r nrow(dt.Theories.Included %>% filter(FrameworkTheoryModel=="Framework"))`), theory (\textit{N} = `r nrow(dt.Theories.Included %>% filter(FrameworkTheoryModel=="Theory"))`), or theoretical model (\textit{N} = `r nrow(dt.Theories.Included %>% filter(FrameworkTheoryModel=="Model"))`; also see Table \@ref(tab:theoryType)).  
...
as.matrix(dt.Theories.Included %>% dplyr::select(Affect, Behavior, Cognition, Desire) %>% mutate_all(~replace(., is.na(.), 0))) %>%
  BinaryCor(., "pearson") %>%
...
scaleDescrCont %>%
  ...
  dplyr::select(characteristic = vars, n, na, mean, `mean win` = win.mean, `mean trim` = trimmed, median,
...
# make crossproduct matrix to condense co-occurrences (off-diagonals) and get frequencies (diagonals)
as.matrix(dt.Scales.Included %>% dplyr::select(Affect, Behavior, Cognition, Desire) %>% mutate_all(~replace(., is.na(.), 0))) %>%
  BinaryCor(., "pearson") %>%
  tibble::rownames_to_column(., var = "Aspect") %>%
  kbl(., 
        #label = "",
...
### Situation  
Acculturation can happen in different life domains [e.g., @Arends-Toth2007; @Zane2004]. We coded which life domains the scales referred to, either as part of subscale labels, factor labels, explicit commentary of the authors, or clear question wordings (see Figure \@ref(fig:ScaleDomainFreq)).  

```{r ScaleDomainFreq, warning=F, fig.cap="Wordcloud of the validation scale domains."}
# remove duplicate domains
UniqueDomains <- dt.Domains %>% 
  filter(Duplicate == "Unique")

# dataframe to compile domain frequency for Review Scales
scalesDomainFreq <- data.frame(UniqueDomains, Frequency = NA)

# count domain frequency in dt.Scales.Included
for (i in 1:nrow(UniqueDomains)) {
  scalesDomainFreq$Frequency[i] <- length(grep(UniqueDomains$Domain[i], dt.Scales.Included$domainScale, value = T))
}

# prepare for ggplot
scalesDomainFreq <- scalesDomainFreq %>%
...
wordcloud(words = scalesDomainFreq$Domain, freq = scalesDomainFreq$Frequency, 
...
Life Domains should still be coded thematically.
</div>
...
dt.Empirical.included %>%
  ...
  summarise(Frequency = n(),
            Percentage = Frequency/nrow(.)*100) %>%
  arrange(desc(Frequency)) %>%
  kbl(., caption = "Empirical Literature: Measure used",
      format = "html",
      digits = 2,
...
empiricalComplexity %>%
  ...
  footnote(general = paste0("M = ",format(round(empiricalComplexityAverage, 2), nsmall=2), ", SD = ", format(round(empiricalComplexitySD, 2), nsmall=2))) %>%
  kable_classic(full_width = F, 
                lightable_options = "hover", 
                html_font = "Cambria")
```
...
# make crossproduct matrix to condense co-occurrences (off-diagonals) and get frequencies (diagonals)
as.matrix(dt.Empirical.included %>% dplyr::select(Affect, Behavior, Cognition, Desire) %>% mutate_all(~replace(., is.na(.), 0))) %>%
  BinaryCor(., "pearson") %>%
  tibble::rownames_to_column(., var = "Aspect") %>%
  kbl(., 
        #label = "",
...
### Situation  
We coded which life domains the authors referred to, either as part of subscale labels, factor labels, explicit commentary of the authors, or clear question wordings to gain an understanding of the situational focus the authors chose(see Figure \@ref(fig:empiricalDomainFreq)).  

```{r empiricalDomainFreq, warning=F, fig.cap="Wordcloud of the life domains studied in empirical studies."}
# unique domains were already calculated

# dataframe to compile domain frequency for Review Scales
empiricalDomainFreq <- data.frame(UniqueDomains, Frequency = NA)

# count domain frequency in dt.Scales.Included
for (i in 1:nrow(UniqueDomains)) {
  empiricalDomainFreq$Frequency[i] <- length(grep(UniqueDomains$Domain[i], dt.Empirical.included$domainScale, value = T))
}

# prepare for ggplot
empiricalDomainFreq <- empiricalDomainFreq %>%
  dplyr::select(Domain, Frequency) %>%
  filter(Domain != "N/A") %>%
  filter(Frequency > 0) %>%
  arrange(Frequency) %>%    # First sort by val. This sort the dataframe but NOT the factor levels
...
# Word cloud frequencies
set.seed(7) # for reproducibility 
wordcloud(words = empiricalDomainFreq$Domain, freq = empiricalDomainFreq$Frequency, 
          min.freq = 1, max.words=200, random.order=FALSE, rot.per=0.35, colors=brewer.pal(8, "Dark2"))
```
...
<br>
<div class="alert alert-danger">
<strong><i class="fa fa-exclamation-triangle"></i> To Do: Sankey here</strong> <br>
...
save(dt.Scales, dt.Scales.Included, 
...
#### Situation  
We also, again, coded which life domains the acculturation measures referred to within each of the disciplines. Again, these were life domains referred to either as part of subscale labels, factor labels, explicit commentary of the authors, or clear question wordings (see Table \@ref(tab:empiricalDisciplineDomainTab) and Figure \@ref(fig:empiricalDisciplineDomainFrequencies)).  

```{r empiricalDisciplineDomainTab, warning=F, message=F}
empiricalDataDisciplines %>%
  dplyr::select(discipline02, domainScale) %>%
  filter(domainScale != "N/A") %>%
...
```{r empiricalDisciplineDomainFrequencies, warning=F, fig.cap="Wordcloud of the scale domains in empirical results.", fig.height=13}
# unique domain already calculated
...
for (i in 1:nrow(UniqueDomains)) {
  PsychDomainFreq$Psychology[i] <- length(grep(UniqueDomains$Domain[i], 
                                               empiricalDataDisciplines$domainScale[empiricalDataDisciplines$discipline02=="Psychology"], 
                                               value = T))
  PsychDomainFreq$`Medicine, Nursing, & Health`[i] <- length(grep(UniqueDomains$Domain[i], 
                                               empiricalDataDisciplines$domainScale[empiricalDataDisciplines$discipline02=="Medicine, Nursing, & Health"], 
                                               value = T))
  PsychDomainFreq$`Social Sciences (miscellaneous)`[i] <- length(grep(UniqueDomains$Domain[i], 
                                               empiricalDataDisciplines$domainScale[empiricalDataDisciplines$discipline02=="Social Sciences (miscellaneous)"], 
                                               value = T))
  PsychDomainFreq$`Multidisciplinary / Crossdisciplinary`[i] <- length(grep(UniqueDomains$Domain[i], 
                                               empiricalDataDisciplines$domainScale[empiricalDataDisciplines$discipline02=="Multidisciplinary / Crossdisciplinary"], 
                                               value = T))
}
...
<!-- ### Exclusions   -->
<!-- Taken together these five reviews collected a total of `r nrow(dt.Scales[dt.Scales$Source != "own review",])` scales. From our own review we added `r nrow(dt.Scales[dt.Scales$Source == "own review",])` additional validation studies. Of these scales we ultimately had to exclude `r nrow(dt.Scales[!is.na(dt.Scales$MissingNote),])`, because they were either not accessible or did not fit the the topic of our review (see Table \@ref(tab:ScalesExclusion)).   -->

<!-- ```{r ScalesExclusions, results='asis'} -->
<!-- data.frame(table(Exclusion = dt.Scales$MissingNote)) %>% -->
<!--   arrange(desc(Freq)) %>% -->
...
<!-- The remaining `r nrow(dt.Scales.Included)` scales are listed in Table \@ref(tab:ScaleTbl).   -->
...
<!--                                          replacement = c("CEL", "MAE", "MAT", "WAL", "ZAN", "OWN"),  -->
<!--                                          vectorize_all = FALSE)) %>% -->
<!--   dplyr::select(Scale, CitationKey, `Source ^a^` = SourceShort,  -->
<!--                 Affect, 	Behavior,	Cognition,	Desire, -->
<!--                 Sample,	IncludesMajority,  -->
<!--                 HostCountry,	OriginCountry) %>% -->
<!--   mutate_at(vars(Affect, 	Behavior,	Cognition, Desire), ~replace_na(., 0)) %>% -->
...
<!-- 
# ```{r child = 'RmdSections/ScaleExclusion.Rmd'}
# rmarkdown::render("RmdSections/ScaleExclusion.Rmd", output_format = latex_document())
# ```
-->
...
<!-- # count domain frequency in dt.Scales.Included -->
<!-- for (i in 1:nrow(UniqueDomains)) { -->
<!--   ScaleDomainFreq$Frequency[i] <- length(grep(UniqueDomains$Domain[i], dt.Scales.Included$domainScale, value = T)) -->
<!-- } -->
...
<!--   dplyr::select(Scale, OriginCountry, HostCountry) -->

<!-- # Host Countries: Find all individual country names and count frequency of occurrence -->
<!-- HostCountryFreq <- data.frame(Country = unique(unlist(strsplit(ScaleMap$HostCountry, ", "))), Frequency = NA, source = "Host") -->
<!-- for (i in 1:nrow(HostCountryFreq)) { -->
<!--   HostCountryFreq$Frequency[i] <- length(grep(HostCountryFreq$Country[i], ScaleMap$HostCountry, value = T)) -->
<!-- } -->
...
<!-- OriginCountryFreq <- data.frame(Country = unique(unlist(strsplit(ScaleMap$OriginCountry, ", "))), Frequency = NA, source = "Origin") -->
<!-- for (i in 1:nrow(OriginCountryFreq)) { -->
<!--   OriginCountryFreq$Frequency[i] <- length(grep(OriginCountryFreq$Country[i], ScaleMap$OriginCountry, value = T)) -->
<!-- } -->
...
<!-- ### Exclusions   -->
<!-- The search produced a total of `r nrow(dt.Empirical)` results. We subsequently screened out results that did not fit into our review. We excluded `r nrow(dt.Empirical)-nrow(dt.Empirical.title)` results in the title screening (for exclusion reasons see Table \@ref(tab:PsychExclusionTitle)) as well as an additional `r nrow(dt.Empirical.title)-nrow(dt.Empirical.abstract)` results during the abstract screening (for exclusion reasons see Table \@ref(tab:PsychExclusionAbstract)).   -->
<!-- ```{r PsychExclusionTitle, results='asis'} -->
<!-- data.frame(table(Exclusion = dt.Empirical$TitleNote)) %>% -->
<!--   arrange(desc(Freq)) %>% -->
...
<!-- data.frame(table(Exclusion = dt.Empirical$AbstractNote)) %>% -->
<!--   arrange(desc(Freq)) %>% -->
...
<!-- Of the remaining `r nrow(dt.Empirical.abstract)` results, `r nrow(dt.Empirical.included)` papers presented empirical work on acculturation and were coded. The `r nrow(dt.Empirical.abstract %>% filter(empirical==0))` non-empirical results were reviews, which were not coded because they did not fit into our coding schema. During the full text coding we excluded an additional `r length(dt.Empirical.included$MissingABCD[dt.Empirical.included$MissingABCD==1])` results were dropped because they were either not relevant or were not accessible (for exclusion reasons see Table \@ref(tab:PsychExclusionCoded)). -->
...
<!-- data.frame(table(dt.Empirical$NoteMissing)) %>% -->
<!--   arrange(desc(Freq)) %>% -->
...
<!-- prismaGr(found = 516, -->
<!--          found_other = 18, -->
<!--          no_dupes = nrow(dt.Empirical),  -->
<!--          screened = nrow(dt.Empirical),  -->
<!--          screen_exclusions = nrow(dt.Empirical)-nrow(dt.Empirical.title),  -->
<!--          full_text = nrow(dt.Empirical.title), -->
<!--          full_text_exclusions = nrow(dt.Empirical.title)-nrow(dt.Empirical.abstract),  -->
<!--          qualitative = nrow(dt.Empirical.abstract),  -->
<!--          quantitative = nrow(dt.Empirical.included), -->
<!--          extra_dupes_box = F, -->
<!--          width = 800, height = 800) %>% -->
<!--   export_svg() %>% -->
...
<!-- ```{r PsychPublicationType} -->
<!-- data.frame(table(dt.Empirical.included$PublicationType)) %>% -->
<!--   arrange(desc(Freq)) %>% -->
...
<!-- ```{r PsychMethod} -->
<!-- data.frame(table(dt.Empirical.included$Method)) %>% -->
<!--   arrange(desc(Freq)) %>% -->
...
<!-- ```{r PsychTerm} -->
<!-- data.frame(table(tolower(dt.Empirical.included$term))) %>% -->
<!--   arrange(desc(Freq)) %>% -->
...
<!-- ```{r PsychMeasure} -->
<!-- data.frame(table(tolower(dt.Empirical.included$MeasureDefinition))) %>% -->
<!--   arrange(desc(Freq)) %>% -->
...
<!-- ggplotly( -->
<!-- ggplot(dt.Empirical.included, aes(x=year)) +  -->
<!--   geom_histogram(bins = length(unique(dt.Empirical.included$year)), fill = "grey14")+ -->
<!--   ylab("Number of Results")+ -->
<!--   xlab("Year") + -->
<!--   ggtitle("Publication Year of Results")+ -->
...
<!-- ggplotly( -->
<!-- ggplot(dt.Empirical.included, aes(x=year, color = PublicationType)) +  -->
<!--   #geom_density(aes(y = ..count..)) + -->
<!--   geom_line(stat='count') + -->
<!--   geom_point(stat='count', size = .9) + -->
...
<!-- ggplotly( -->
<!-- ggplot(dt.Empirical.included, aes(x=year, color = Method)) +  -->
<!--   #geom_density(aes(y = ..count..)) + -->
<!--   geom_line(stat='count') + -->
<!--   geom_point(stat='count', size = .9) + -->
<!--   #facet_grid(.~PublicationType) + -->
...
<!-- ```{r descriptivs, include=FALSE} -->
<!-- summarytools::freq(dt.Empirical.included$MeasurementLevels, order = "freq") -->

<!-- summarytools::freq(dt.Empirical.included$IncludesMajority, order = "freq") -->

<!-- summarytools::freq(dt.Empirical.included$VariableType, order = "freq") -->

<!-- summarytools::freq(dt.Empirical.included$Sample, order = "freq") -->

<!-- summarytools::freq(dt.Empirical.included$domainScale, order = "freq") -->

<!-- summarytools::freq(dt.Empirical.included$domainPaper, order = "freq") -->

<!-- summarytools::freq(dt.Empirical.included$MigrationTime, order = "freq") -->
<!-- ``` -->
...
<!-- # tally different samples -->
<!-- PsychSampleFreq <- as.data.frame(table(Sample = dt.Empirical.included$Sample)) %>% -->
<!--   arrange(Freq) %>%    # First sort by val. This sort the dataframe but NOT the factor levels -->
<!--   mutate(Sample=factor(Sample, levels=Sample)) # update factor levels -->
...
<!-- data.frame(table(tolower(dt.Empirical.included$MigrationTime))) %>% -->
<!--   arrange(desc(Freq)) %>% -->
...
<!-- ```{r PsychMayority} -->
<!-- # tally different migration times -->
<!-- data.frame(table(dt.Empirical.included$IncludesMajority)) %>% -->
...
<!-- # Count the times each dimension is measured -->
<!-- PsychDimensionFreq <- dt.Empirical.included %>% -->
<!--   dplyr::select(Affect, Behavior, Cognition, Desire) %>% -->
<!--   mutate_at(vars(Affect, Behavior, Cognition, Desire), ~replace_na(., 0)) %>% -->
<!--   colSums(., na.rm = FALSE, dims = 1)  -->
...
<!-- # frequency of unique combinations -->
<!-- PsychABCD_comb <- dt.Empirical.included %>% -->
<!--   dplyr::select(Affect, Behavior, Cognition, Desire) %>% -->
<!--   #mutate_at(vars(Affect, Behavior, Cognition, Desire), ~replace_na(., "")) %>% -->
<!--   group_by(Affect, Behavior, Cognition, Desire) %>% -->
<!--   summarise(count = n()) -->
...
<!-- # make crossproduct matrix to condense co-occurrences (off-diagonals) and get frequencies (diagonals) -->
<!-- PsychDim <- as.matrix(dt.Empirical.included %>%  -->
<!--                  dplyr::select(Affect, Behavior, Cognition, Desire) %>%  -->
<!--                  mutate_at(vars(Affect, Behavior, Cognition, Desire), ~replace_na(., 0))) -->
...
<!-- # count domain frequency in dt.Scales.Included -->
<!-- for (i in 1:nrow(UniqueDomains)) { -->
<!--   PsychDomainFreq$Frequency[i] <- length(grep(UniqueDomains$Domain[i], dt.Empirical.included$domainScale, value = T)) -->
<!-- } -->
...
<!-- # count domain frequency in dt.Scales.Included -->
<!-- for (i in 1:nrow(UniqueDomains)) { -->
<!--   PsychTopicFreq$Frequency[i] <- length(grep(UniqueDomains$Domain[i], dt.Empirical.included$domainPaper, value = T)) -->
<!-- } -->
...
<!-- # subset relevant variables -->
<!-- PsychMap <- dt.Empirical.included %>% -->
...
<!-- ```{r PsychAnalysis} -->
<!-- # tally different migration times -->
<!-- data.frame(table(dt.Empirical.included$MainAnalysis)) %>% -->
<!--   arrange(desc(Freq)) %>% -->
...
<!-- ```{r PsychVarType} -->
<!-- # tally different migration times -->
<!-- data.frame(table(dt.Empirical.included$VariableType)) %>% -->
<!--   arrange(desc(Freq)) %>% -->
...
<!-- # Host Countries: Find all individual country names and count frequency of occurrence -->
<!-- PsychHostCountryFreq <- data.frame(Country = unique(unlist(strsplit(empiricalDataDisciplines$HostCountry, ", "))), source = "Host") -->
<!-- for (i in 1:nrow(PsychHostCountryFreq)) { -->
<!--   PsychHostCountryFreq$Psychology[i] <- length(grep(PsychHostCountryFreq$Country[i],  -->
<!--                                                empiricalDataDisciplines$HostCountry[empiricalDataDisciplines$discipline02=="Psychology"],  -->
<!--                                                value = T)) -->
<!--   PsychHostCountryFreq$`Medicine, Nursing, & Health`[i] <- length(grep(PsychHostCountryFreq$Country[i],  -->
<!--                                                empiricalDataDisciplines$HostCountry[empiricalDataDisciplines$discipline02=="Medicine, Nursing, & Health"],  -->
<!--                                                value = T)) -->
<!--   PsychHostCountryFreq$`Social Sciences (miscellaneous)`[i] <- length(grep(PsychHostCountryFreq$Country[i],  -->
<!--                                                empiricalDataDisciplines$HostCountry[empiricalDataDisciplines$discipline02=="Social Sciences (miscellaneous)"],  -->
<!--                                                value = T)) -->
<!--   PsychHostCountryFreq$`Multidisciplinary / Crossdisciplinary`[i] <- length(grep(PsychHostCountryFreq$Country[i],  -->
<!--                                                empiricalDataDisciplines$HostCountry[empiricalDataDisciplines$discipline02=="Multidisciplinary / Crossdisciplinary"],  -->
<!--                                                value = T)) -->
<!-- } -->


<!-- # Origin Countries: Find all individual country names and count frequency of occurrence -->
<!-- PsychOriginCountryFreq <- data.frame(Country = unique(unlist(strsplit(empiricalDataDisciplines$OriginCountry, ", "))),  -->
<!--                                      source = "Origin") -->
<!-- for (i in 1:nrow(PsychOriginCountryFreq)) { -->
<!--   PsychOriginCountryFreq$Psychology[i] <- length(grep(PsychOriginCountryFreq$Country[i],  -->
<!--                                                empiricalDataDisciplines$OriginCountry[empiricalDataDisciplines$discipline02=="Psychology"],  -->
<!--                                                value = T)) -->
<!--   PsychOriginCountryFreq$`Medicine, Nursing, & Health`[i] <- length(grep(PsychOriginCountryFreq$Country[i],  -->
<!--                                                empiricalDataDisciplines$OriginCountry[empiricalDataDisciplines$discipline02=="Medicine, Nursing, & Health"],  -->
<!--                                                value = T)) -->
<!--   PsychOriginCountryFreq$`Social Sciences (miscellaneous)`[i] <- length(grep(PsychOriginCountryFreq$Country[i],  -->
<!--                                                empiricalDataDisciplines$OriginCountry[empiricalDataDisciplines$discipline02=="Social Sciences (miscellaneous)"],  -->
<!--                                                value = T)) -->
<!--   PsychOriginCountryFreq$`Multidisciplinary / Crossdisciplinary`[i] <- length(grep(PsychOriginCountryFreq$Country[i],  -->
<!--                                                empiricalDataDisciplines$OriginCountry[empiricalDataDisciplines$discipline02=="Multidisciplinary / Crossdisciplinary"],  -->
<!--                                                value = T)) -->
<!-- } -->
