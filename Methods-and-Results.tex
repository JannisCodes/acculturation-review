...
(\textit{m} = 2.61, '\textit{sd} = 0.90; also see Table
\ref{tab:CombinedElementComplexity}). Yet they tend to focus on more
...
Table \ref{tab:CombinedElementComplexity}). Thus, most scales measure
...
(\textit{sd} = 0.58; also see Table
\ref{tab:CombinedElementComplexity}). Thus, not a single study measured
...
\textbackslash caption\{Literature Levels: Average number of additional
aspects considered when the aspect was included for theoretical,
methodological, and broader empirical literature {[}Mean ± 95\%CI{]}\}
